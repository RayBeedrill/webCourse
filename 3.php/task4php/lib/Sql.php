...
class Sql
{
	...
	*Function for seting value of select action
	 */
	public function setSelectVal($cols)
	{
		if("*" !== $cols)
		{
			$this->selectVal = " SELECT " .  $cols;
			
		}
		else
		{
			$this->selectVal = SEL_ALL_ERR;
		}
	...
	public function sendQueryString()
	{
		...
		switch(true)
		{
			case !empty($this->selectVal) && $this->selectVal !== SEL_ALL_ERR:
				$query =  $this->selectVal .  $this->fromVal .  $this->whereVal ;  
				break;
			case !empty($this->insertVal):
				$query = $this->insertVal . $this->valuesVal ; 
				break;
			case !empty($this->updateVal) && $this->updateVal !== SEL_ALL_ERR:
				$query = $this->updateVal . $this->setVal . $this->whereVal ; 
				break;
			case !empty($this->deleteVal) && $this->deleteVal !== SEL_ALL_ERR:
				$query = $this->deleteVal . $this->whereVal ; 
				break;    
		}
		return $query;
	}
